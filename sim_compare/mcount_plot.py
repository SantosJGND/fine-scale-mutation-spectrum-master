...
for strata in plot_data.keys():
    ...
    for i in batch_dict.keys():
        ...
        plt.xlabel(xlab.format(strata) + ' {} comparisons'.format(len(batch_dict[i])))
        plt.ylim(0,1.03)
        plt.ylabel(ylab)
        plt.title(i)
        
        plt.savefig('{}_{}.png'.format(i, strata),bbox_inches='tight')
    ...
    plt.plot(*append_all)
    plt.xlabel(xlab.format(strata))
    plt.ylim(0,1.03)
    plt.ylabel(ylab)
    plt.title('combined stats')

    plt.savefig('combined_{}.png'.format(strata),bbox_inches='tight')
