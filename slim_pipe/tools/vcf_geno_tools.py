...
def read_geno_nanumv3(filename,header_info= 9,phased= False):
    '''
    read vcf file simple, 
    phased= False sums alleles, phased= True returns haplotypes.
    row_info_deprecated from v1.
    - v3: tackle reference alleles with commma, indicative of mulitple segregating alleles at a locus.
            specific to SLiM produced VCFs. ignore for now.
    '''
    info_summ= {}
    ...
    for line in Input:    
        ...
        if d > 0:
            ...
            if ',' in alt:
                new_alt= alt.split(',')
                new_alt= [x for x in new_alt if x != info[3]]
                if len(new_alt):
                    info[4]= new_alt[0]
                else: ignore= 1
            ...
            for ind in range(header_len,len(line)):
                ...
                if phased:
                    ...
                    seq.append(sum([x for x in alleles if x < 2]))
