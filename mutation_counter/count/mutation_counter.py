...
class MutationCounter:
    ...
    def configure(self, line, dir_launch='..'):
    ...
    def process_line(self, line,filter= True,proper= False,correct= False,print_c= True,bed_filter=[]):
        s=line.strip(b'\n').split(b'\t')
        ...
        if len(s[3]+s[4])==2 and s[6] in filter_flags and s[3] in b'ACGT' and s[4] in b'ACGT':
            ...
            haplotypes= [x.count(b'1') for x in haplotypes]
            ...
            if min(count_der,self.n_lineages-count_der)>1:
                ...
                i=0
                der_observed=0
                count = {population.encode(): 0 for population in populations}
                ...
                for pop in populations:
                    if count[pop.encode()]>0:
                        for included_region in self.included_regions:
                            
                            included_region.update_counts(pos, this_mut, pop, count[pop.encode()])
